# SP2 Resit Auction

<img width="2559" height="1317" alt="sp2-resit-screenshot" src="https://github.com/user-attachments/assets/2d209a08-5ac1-4de1-b735-774fc39a439c" />

A modern auction web application built for the Noroff Frontend Development Semester Project 2 Resit.
Users can browse listings, create auctions, and place bids on items.

---

## Description

**SP2 Resit Auction** is a full-featured auction platform developed as part of the Noroff Frontend Development course.

The application allows users to:

- Browse available auction listings
- View detailed item pages
- Place bids on items
- Create their own listings (registered users)
- Manage their profile and avatar

The project integrates with the **Noroff Auction API** for authentication, listings, and bidding functionality.

---

## Key Features

- Browse auction listings
- Search through listings
- View detailed listing pages
- Create new auction listings
- Place bids on items
- Register and login with a student email
- View and update user profile
- Update avatar
- View bids on listings
- Fully responsive design using **Bootstrap**

---

## Built With

- **HTML5**
- **JavaScript (Vanilla JS)**
- **Bootstrap 4/5**
- **Noroff Auction API**
- **Fetch API**
- **Node.js**

---

## Getting Started

### Installing

Clone the repository:

```bash
git clone https://github.com/OH2021/SP2-Resit.git
```

Navigate into the project folder:

```bash
cd SP2-Resit
```

Install dependencies:

```bash
npm install
```

### Running

Run the development environment:

```bash
npm run dev
```

The application will run locally based on your configuration.

---

## Project Resources

- **Gantt Chart:** (https://learning.noroff.no/mod/assign/view.php?id=28119&action=editsubmission#
 trello_gantt_placker_export_202604021133.pdf.url…]())

- **Design Prototype:** (https://xd.adobe.com/view/b19b36d0-b016-4393-a0a0-331ddfd7a294-e4c8/)
- **Style Guide:** (https://github.com/OH2021/SP2-Resit/blob/main/STYLE_GUIDE.md)
- **Kanban Board:** (https://github.com/users/OH2021/projects/4/views/1)
- **Repository:** (https://github.com/OH2021/SP2-Resit)
- **Hosted Demo (Live Site):** (https://sp2-resit.netlify.app/index.html)

---

## Contributing

This project was developed as part of a school assignment and is not currently open for public contributions.

To suggest improvements:

1. Fork the repository
2. Create a new branch
3. Submit a pull request for review

---

## Contact

- **GitHub:** [ole-henrik-haug](https://github.com/OH2021)
- **Email:** [ole.henrik.haug@gmail.com](mailto:ole.henrik.haug@gmail.com)
- **LinkedIn:** [Ole Henrik Haug](https://www.linkedin.com/in/ole-henrik-haug-8a17751b9/)

---

## License

This project was created for educational purposes as part of the Noroff Frontend Development Program.

---

## Acknowledgments

- Noroff Frontend Development Program
- Noroff Auction API documentation
- **Bootstrap** documentation
- JavaScript documentation
